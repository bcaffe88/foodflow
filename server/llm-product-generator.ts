...
export async function generateProductsByCategory(
  req: CategoryGenerationRequest
): Promise<GenerationResponse> {
  const prompt = `Você é um especialista em cardápios de restaurantes. Gere produtos para o seguinte estabelecimento:

Tipo de Estabelecimento: ${req.establishmentType}
Categoria de Produtos: ${req.category}

Gere uma lista de até 10 produtos típicos para esta categoria neste tipo de estabelecimento. Para CADA produto, forneça:
- Nome do produto (em português)
- Descrição breve (1-2 linhas, máximo 100 caracteres)
- Preço sugerido em BRL
- Categoria (a mesma fornecida)
- Tempo de preparo em minutos
...
IMPORTANTE: Responda APENAS em JSON válido (sem markdown), sem explicações antes ou depois. Seguindo este formato exato:
{
  ...
  try {
    ...
    const products = (parsed.products || []).map((p: any) => ({
      ...
      description: String(p.description || "").substring(0, 500),
      price: Math.max(1, Math.min(1000, Number(p.price) || 10)),
      category: String(p.category || req.category).substring(0, 50),
      preparationTime: Math.max(5, Math.min(120, Number(p.preparationTime) || 15)),
      isAvailable: p.isAvailable !== false,
    }));
