...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get restaurant dashboard data (ENHANCED with queue panel info)
  app.get("/api/restaurant/dashboard", 
    ...
    async (req: AuthRequest, res) => {
      try {
        ...
        // Enrich pending orders with payment info and customer details
        const enrichedPendingOrders = await Promise.all(
          pendingOrders.map(async (order) => {
            // Get payment info
            const paymentData = await storage.getPaymentByOrderId?.(order.id);
            return {
              id: order.id,
              orderNumber: order.id.slice(-6).toUpperCase(), // Order display number
              ...
              paymentStatus: paymentData?.status || (order.paymentMethod === 'cash' ? 'pending' : 'pending'),
        ...
        res.json({
          ...
          totalRevenue,
          // Queue panel with complete order information
          queuePanel: {
            orders: enrichedPendingOrders.slice(0, 20),
            total: pendingOrders.length,
          },
          // Legacy response for backward compatibility
          pendingOrders: enrichedPendingOrders.slice(0, 10),
        });
      } catch (error) {
  ...
  app.post("/api/restaurant/orders/:id/cancel",
    authenticate,
    requireRole("restaurant_owner"),
    requireTenantAccess,
    async (req: AuthRequest, res) => {
      try {
        ...
        res.json({ success: true, order: cancelledOrder, cancelled: true, reason });
      } catch (error) {
  ...
  // ============================================================================
  // PHASE 7 TOOLS - Menu, History, Promos, Address, Status
  // ============================================================================
  ...
  app.get("/api/restaurant/menu",
    ...
    async (req: AuthRequest, res) => {
      try {
        ...
        let products = await storage.getProductsByTenant(tenantId);
        
        if (category) {
          products = products.filter(p => p.categoryId === category || p.category === category);
        }
  ...
  app.get("/api/customer/:phone/history",
    async (req, res) => {
      try {
        ...
        if (!phone || !tenantId) {
          return res.status(400).json({ error: "Phone and tenantId required" });
        }
        ...
        const totalSpent = recentOrders.reduce((sum, o) => sum + parseFloat(o.total as string), 0);
        const totalOrders = recentOrders.length;
        const favoriteItems = recentOrders
  ...
  app.get("/api/promotions/active",
    async (req, res) => {
      try {
        ...
        // Get active promotions from storage or use mock data
        const promotions = [
          {
            id: "promo-1",
            name: "Welcome 20% Off",
          ...
          {
            id: "promo-2",
            name: "Free Delivery on Orders Over R$50",
          ...
          {
            id: "promo-3",
            name: "Bundle Combo - Pizza + Drink",
            description: "Save R$10 on bundle",
            type: "fixed",
            value: 10,
            applicable: true,
          },
        ...
        res.json({
          success: true,
          promotions: promotions.filter(p => p.applicable),
  ...
  app.post("/api/delivery/validate-address",
    async (req, res) => {
      try {
        const { address, restaurantLat, restaurantLng, tenantId } = req.body;

        if (!address || restaurantLat === undefined || restaurantLng === undefined) {
          return res.status(400).json({ error: "Address and restaurant coordinates required" });
        }

        // Validate address using Google Maps (mock for now)
        const isValid = address.length > 5;
        
        // Calculate estimated delivery
        const distanceKm = Math.random() * 15 + 0.5; // Mock distance 0.5-15.5 km
        const estimatedMinutes = Math.ceil(15 + (distanceKm / 4) * 15);
        const deliveryFee = parseFloat((Math.random() * 10 + 3).toFixed(2)); // Mock fee R$3-13
      ...
      try {
        ...
        res.json({
          ...
          order: {
            ...
            statusTimeline,
            estimatedDeliveryTime: new Date(Date.now() + 45 * 60000).toISOString(), // 45 mins
            orderDetails: {
